...
| 우선 | 모듈 | 매칭 프론트 |
|---|---|---|
| **P0** | Auth (가입 / OTP / 이메일 OTP 로그인 / PIN 2FA / JWT / Refresh) | chon-pass-login.html |
| **P1** | Users · Contacts (해시 기반 연락처 동기화) | 프로필, 모임 |
| **P2** | Relations (가족 tier · 모임 1·2-hop · 학급 role) | 가계도·모임·학급증 |
| **P3** | Posts/Feed (작성·피드·좋아요·댓글) | pass-v2 피드 |
| **P4** | Chat (1:1 대화방·메시지·읽음) | pass-v2 채팅 |
...
기존 PASS 프론트 (`../frontend/chon-pass-*.html`)는 sessionStorage·localStorage로만 동작하는 데모.
이 백엔드와 연결하려면:
1. `chon-pass-login.html`의 `doLogin`, `confirmSignup`, `verifyCode` 등을 fetch로 교체
